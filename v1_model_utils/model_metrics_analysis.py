...
def calculate_OSI_DSI(
    firingRates,
    network,
    session="drifting_gratings",
    DG_angles=range(0, 360, 45),
    n_selected_neurons=None,
    core_radius=None,
    ...
    if n_selected_neurons is not None:
        pop_names = other_v1_utils.pop_names(
            network, n_selected_neurons=n_selected_neurons
        )
    ...
    if os.path.exists(os.path.join(directory, f"v1_features_df.csv")):
        osi_dsi_df = pd.read_csv(
            os.path.join(directory, f"v1_features_df.csv"), sep=" "
        )
    else:
    ...
    if session == "drifting_gratings":
        ...
        if n_trials >= 8:
            TuningAngleEstimation = PreferredTuningAngleAnalysis(
                firing_rates=firingRates,
                orientations=DG_angles,
                preferred_orientations=network["tuning_angle"],
            )
            new_tuning_angles, preferred_angle_rates = (
                TuningAngleEstimation.calculate_tuning_angle()
            )
            osi_dsi_df["max_mean_rate(Hz)"] = preferred_angle_rates
            osi_dsi_df["preferred_angle"] = new_tuning_angles
        else:
            osi_dsi_df["preferred_angle"] = DG_angles[
                np.argmax(all_direction_rates, axis=0)
            ]
        ...
        if n_angles >= 8:
            ...
            dsi = np.where(
                denominator != 0,
                np.abs((all_direction_rates * np.exp(1j * phase_rad)).sum(axis=0))
                / denominator,
                np.nan,
            )
            osi = np.where(
                denominator != 0,
                np.abs((all_direction_rates * np.exp(2j * phase_rad)).sum(axis=0))
                / denominator,
                np.nan,
            )
            osi_dsi_df["OSI"] = osi
    ...
    if save_df:
        ...
        osi_dsi_df.to_csv(
            os.path.join(directory, f"v1_features_df.csv"), sep=" ", index=False
        )
...
def compute_ks_statistics(df, metric="Ave_Rate(Hz)", min_n_sample=15):
    ...
    for cell_type in cell_types:
        ...
        df1 = df.loc[
            (df["cell_type"] == cell_type) & (df["data_type"] == "V1 GLIF model"),
            metric,
        ]
        df2 = df.loc[
            (df["cell_type"] == cell_type) & (df["data_type"] == "Neuropixels"), metric
        ]
        # Drop NA values
...
class PreferredTuningAngleAnalysis:
    ...
    def calculate_orientation_vectors(self, responses, orientations):
        ...
        orientation_vectors = np.stack(
            (orientation_vectors_x, orientation_vectors_y), axis=-1
        )
    ...
    def calculate_tuning_angle(self):
        for neuron_idx in range(self.firing_rates.shape[2]):
            ...
            orientation_vectors = self.calculate_orientation_vectors(
                neuron_responses_all_trials, self.orientations
            )
            ...
            for pref_orientation in initial_pref_orientations:
                ...
                for sigma in initial_sigmas:
                    ...
                    try:
                        ...
                        fit_error = np.sum(
                            (
                                self.double_gaussian(self.orientations, *popt)
                                - neuron_responses
                            )
                            ** 2
                        )
            ...
            if best_fit is not None:
                pref_angle = (
                    best_fit[3] if best_fit[2] < best_fit[1] else best_fit[3] - 180
                )
...
class ModelMetricsAnalysis:
    ...
    def __init__(
        ...
        drifting_gratings_init=None,
        drifting_gratings_end=None,
        spontaneous_init=None,
        spontaneous_end=None,
        core_radius=400,
        save_df=False,
        df_directory="Metrics_analysis",
    ):
        ...
        if self.drifting_gratings_init is not None:
            firingRates = calculate_Firing_Rate(
                spikes,
                stimulus_init=self.drifting_gratings_init,
                stimulus_end=self.drifting_gratings_end,
                temporal_axis=2,
            )
            self.metrics_df = calculate_OSI_DSI(
                firingRates,
                self.network,
                session="drifting_gratings",
                DG_angles=DG_angles,
                n_selected_neurons=n_neurons_plot,
        ...
        if self.spontaneous_init is not None:
            spontaneous_firingRates = calculate_Firing_Rate(
                spikes,
                stimulus_init=self.spontaneous_init,
                stimulus_end=self.spontaneous_end,
                temporal_axis=2,
            )
            self.metrics_df = calculate_OSI_DSI(
                spontaneous_firingRates,
                self.network,
                session="spontaneous",
                DG_angles=DG_angles,
                n_selected_neurons=n_neurons_plot,
...
class MetricsBoxplot:
    ...
    def get_osi_dsi_df(self, metric_file, data_source_name="", feature="", data_dir=""):
        ...
        if data_dir == "Neuropixels_data":
            ...
            df = pd.read_csv(
                f"{data_dir}/{metric_file}",
                index_col=0,
                sep=" ",
                usecols=features_to_load,
            ).dropna(how="all")
            df = df[df["cell_type"].notna()]
            df["cell_type"] = df["cell_type"].apply(
                self.neuropixels_cell_type_to_cell_type
            )
        ...
        if "max_mean_rate(Hz)" in df.columns:
            df.rename(
                columns={"max_mean_rate(Hz)": "Rate at preferred direction (Hz)"},
                inplace=True,
            )
    ...
    def plot(
        ...
        self.osi_dsi_dfs.append(
            self.get_osi_dsi_df(
                metrics_df, data_source_name="V1 GLIF model", data_dir=self.save_dir
            )
        )
        self.osi_dsi_dfs.append(
            self.get_osi_dsi_df(
                f"v1_OSI_DSI_DF.csv",
                data_source_name="Neuropixels",
                data_dir="Neuropixels_data",
            )
        )
        ...
        self.osi_dsi_dfs.append(
            self.get_osi_dsi_df(
                f"V1_OSI_DSI_DF_pop_name.csv",
                data_source_name="Baseline model",
                data_dir="NEST_metrics",
            )
        )
        ...
        if axis is None:
            ...
            plt.savefig(
                os.path.join(self.save_dir, self.filename + ".png"),
                dpi=300,
                transparent=False,
            )
            plt.close()
...
def plot_one_metric(
    ax, df, metric_name, ylim, cpal=None, hue_order=None, similarity_score=None
):
...
class OneShotTuningAnalysis:
    def __init__(
        ...
        data_dir="GLIF_network",
        directory="",
        drifting_gratings_init=50,
        drifting_gratings_end=550,
        core_radius=400,
    ):
        self.n_neurons = network["n_nodes"]
        self.network = network
    ...
    def __call__(self, spikes, current_orientation):
        ...
        self.firing_rate = calculate_Firing_Rate(
            spikes,
            stimulus_init=self.drifting_gratings_init,
            stimulus_end=self.drifting_gratings_end,
            temporal_axis=2,
        ...
        self.pop_names = other_v1_utils.pop_names(
            self.network, n_selected_neurons=n_neurons_plot
        )
        self.cell_types = np.array(
            [MetricsBoxplot.pop_name_to_cell_type(x) for x in self.pop_names]
        )

        # Get the orientation angles and orientation assignments using the 'assign_orientation' method
        self.orientation_angles, self.orientation_assignments = self.assign_orientation(
            self.tuning_angles, n_neurons_plot
        )
    ...
    def plot_tuning_curves(self, epoch, remove_zero_rate_neurons=True):
        ...
        fig, axs = plt.subplots(
            nrows=nrows, ncols=ncols, figsize=(20, 25)
        )  # Share x-axis among all plots
        ...
        for cell_id, cell_type in enumerate(unique_cell_types):
            ...
            for i, angle in enumerate(self.orientation_angles):
                ...
                angle_mask = np.logical_and(
                    self.orientation_assignments[:, i], cell_type_mask
                )
            ...
            ax.errorbar(
                ...
                color="black",
                fmt="-o",
                label=f"{cell_type} tuning",
            )

            # Add vertical line for the current orientation
            ax.axvline(
                x=self.current_orientation,
                color="red",
                linestyle="--",
                label=f"Current orientation: {self.current_orientation:.2f}\u00b0",
            )
        ...
        plt.savefig(
            os.path.join(path, f"epoch_{epoch}.png"), dpi=300, transparent=False
        )
        plt.close()
    ...
    def plot_max_rate_boxplots(self, epoch, remove_zero_rate_neurons=False, axis=None):
        ...
        firing_rates_df = pd.DataFrame(
            {
                "pop_name": self.pop_names,
                "Rate at preferred direction (Hz)": np.full(
                    len(self.firing_rate), np.nan
                ),
            }
        )
        ...
        for i, cell_type in enumerate(unique_cell_types):
            ...
            firing_rates_df.loc[mask, "Rate at preferred direction (Hz)"] = (
                preferred_firing_rate
            )

        if remove_zero_rate_neurons:
            firing_rates_df = firing_rates_df[
                firing_rates_df["Rate at preferred direction (Hz)"] != 0
            ]
