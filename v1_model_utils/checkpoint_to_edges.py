...
if debug:
    args = argparse.Namespace(
        ...
        checkpoint_name="V1_GLIF_model/Simulation_results/v1_66652/b_7khs/Best_model",
        network_dir="V1_GLIF_model/GLIF_network/network",
        suffix="checkpoint",
    )
else:
    # let's use the first argument as the checkpoint name.
    parser = argparse.ArgumentParser(
        description="Convert a TF checkpoint file to a BMTK edge file."
    )
    parser.add_argument(
        "checkpoint_name", type=str, help="The name of the checkpoint file."
    )
    ...
    parser.add_argument(
        "--network_dir",
        type=str,
        default="GLIF_network/network",
        help="The directory of the network files.",
    )
    parser.add_argument(
        "--suffix",
        type=str,
        default="checkpoint",
        help="The suffix to be added to the edge file.",
    )
...
# get the value.
voltage_scale = ckpt.get_tensor(
    "model/layer_with_weights-0/cell/voltage_scale/.ATTRIBUTES/VARIABLE_VALUE"
)

# also, the recurrent weight values
recurrent_weights = ckpt.get_tensor(
    "model/layer_with_weights-0/cell/recurrent_weight_values/.ATTRIBUTES/VARIABLE_VALUE"
)
recurrent_weights = recurrent_weights.flatten()


# connection indices
indices = ckpt.get_tensor(
    "model/layer_with_weights-0/cell/recurrent_indices/.ATTRIBUTES/VARIABLE_VALUE"
)
...
non_agree_frac = np.sum(((orig_weights * checkpoint_weights_reverted) < 0)) / len(
    orig_weights
)
# non_agree_frac = np.sum(np.sign(orig_weights) != np.sign(checkpoint_weights_reverted)) / len(orig_weights)
print(
    f"*** Percentage of weights that do not agree in sign: {(non_agree_frac * 100):.2f}% ***"
)
...
try:
    bkg_weights = ckpt.get_tensor(
        "model/layer_with_weights-0/cell/bkg_input_weights/.ATTRIBUTES/VARIABLE_VALUE"
    )
    bkg_indices = ckpt.get_tensor(
        "model/layer_with_weights-0/cell/bkg_input_indices/.ATTRIBUTES/VARIABLE_VALUE"
    )
except RuntimeError:  # for the case of the old model
    logging.warning("Old model detected. Reading the old variable names...")
    bkg_weights = ckpt.get_tensor(
        "model/layer_with_weights-1/_bkg_weights/.ATTRIBUTES/VARIABLE_VALUE"
    )
    bkg_indices = ckpt.get_tensor(
        "model/layer_with_weights-1/_bkg_indices/.ATTRIBUTES/VARIABLE_VALUE"
    )
