...
def pop_names(
    ...
    with h5py.File(path_to_h5, mode="r") as node_h5:
        ...
        true_pop_names = np.array(
            [node_type_id_to_pop_name[nid] for nid in node_type_ids]
        )

    if core_radius is not None:
        selected_mask = isolate_core_neurons(
            network, radius=core_radius, data_dir=data_dir
        )
    elif n_selected_neurons is not None:
        selected_mask = isolate_core_neurons(
            network, n_selected_neurons=n_selected_neurons, data_dir=data_dir
        )
    else:
...
def connection_type_ids(
    network, core_radius=None, data_dir="GLIF_network", return_names=False
):
    ...
    if return_names:
        ...
        names["all_names"] = [
            f'{names["pre"][i]} -> {names["post"][i]}' for i in range(len(all_pop_ids))
        ]
...
def isolate_core_neurons(
    network, radius=None, n_selected_neurons=None, data_dir="GLIF_network"
):
    path_to_h5 = os.path.join(data_dir, "network/v1_nodes.h5")
    with h5py.File(path_to_h5, mode="r") as node_h5:
...
def optimizers_match(current_optimizer, checkpoint_directory):
    current_optimizer_vars = {
        v.name: v.shape.as_list() for v in current_optimizer.variables()
    }
    checkpoint_vars = tf.train.list_variables(checkpoint_directory)
    checkpoint_optimizer_vars = {
        name.split("/.ATTRIBUTES")[0]: value
        for name, value in checkpoint_vars
        if "optimizer" in name
    }
    if (
        "optimizer/loss_scale/current_loss_scale" in checkpoint_optimizer_vars
        or "optimizer/loss_scale/good_steps" in checkpoint_optimizer_vars
    ):
        if len(current_optimizer_vars) != len(checkpoint_optimizer_vars) - 3:
            print(
                "Checkpoint optimizer variables do not match the current optimizer variables.. Renewing optimizer..."
            )
            return False
        else:
...
class SaveSimDataHDF5:
    def __init__(
        self, flags, keys, data_path, network, save_core_only=True, dtype=np.float16
    ):
        self.keys = keys
...
def load_simulation_results(
    ...
    data = {
        key: (
            np.empty((n_simulations, simulation_length, n_input), np.uint8)
            if key == "z_lgn"
            else (
                np.empty((n_simulations, simulation_length, n_neurons), np.uint8)
                if key == "z"
    ...
    for i in range(first_simulation, last_simulation):
        for key, value in data.items():
            ...
            if key in ["z", "z_lgn"]:
                data[key][(i - first_simulation) : (i + 1 - first_simulation), :, :] = (
...
def load_simulation_results_hdf5(
    full_data_path, n_simulations=None, skip_first_simulation=False, variables=None
):
    ...
    with h5py.File(full_data_path, "r") as f:
        ...
        for key in variables:
            if key in ["z", "z_lgn"]:
                data[key] = np.array(
                    dataset[key][first_simulation:last_simulation, :, :]
                ).astype(np.uint8)
            else:
                data[key] = np.array(
                    dataset[key][first_simulation:last_simulation, :, :]
