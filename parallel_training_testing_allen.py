...
# Define arguments
parser.add_argument(
    "--task_name", default="drifting_gratings_firing_rates_distr", type=str
)
...
parser.add_argument("--rate_cost", default=100.0, type=float)  # 100
parser.add_argument("--voltage_cost", default=1.0, type=float)
parser.add_argument("--sync_cost", default=1.0, type=float)
parser.add_argument("--osi_cost", default=1.0, type=float)
parser.add_argument("--osi_loss_subtraction_ratio", default=0.0, type=float)
...
parser.add_argument(
    "--n_runs", default=1, type=int
)  # number of runs with n_epochs each, with an osi/dsi evaluation after each
...
def main():
    ...
    for name, value in vars(flags).items():
        if value != parser.get_default(name) and name in [
            "n_input",
            "core_only",
            "connected_selection",
            "random_weights",
    ...
    initial_evaluation_command = evaluation_commands + [
        "-o",
        f"Out/{sim_name}_{v1_neurons}_initial_test.out",
        "-e",
        f"Error/{sim_name}_{v1_neurons}_initial_test.err",
        "-J",
        f"{sim_name}_initial_test",
    ]
    if initial_benchmark_model:
        initial_evaluation_script = (
            evaluation_script
            + f"--seed {flags.seed} --ckpt_dir {logdir}  --run_session {-1} --  {initial_benchmark_model}"
        )
    else:
        initial_evaluation_script = (
            evaluation_script
            + f"--seed {flags.seed} --ckpt_dir {logdir}  --run_session {-1}"
        )

    initial_evaluation_command = (
        initial_evaluation_command + ["--wrap"] + [initial_evaluation_script]
    )
    ...
    for i in range(flags.n_runs):
        ...
        if i == 0:
            new_training_command = training_commands + [
                "-o",
                f"Out/{sim_name}_{v1_neurons}_train_{i}.out",
                "-e",
                f"Error/{sim_name}_{v1_neurons}_train_{i}.err",
                "-J",
                f"{sim_name}_train_{i}",
            ]
            if initial_benchmark_model:
                new_training_script = (
                    training_script
                    + f"--seed {flags.seed + i} --ckpt_dir {logdir} --run_session {i} --restore_from {initial_benchmark_model} "
                )
            else:
                new_training_script = (
                    training_script
                    + f" --seed {flags.seed + i} --ckpt_dir {logdir} --run_session {i}"
                )
            new_training_command = (
                new_training_command + ["--wrap"] + [new_training_script]
            )
        ...
        if (
            flags.n_runs == 1
        ):  # the run is a single run, no need to submit evaluation jobs. osi_dsi will be evaluated at the end of training run
            continue
        else:
            new_evaluation_command = evaluation_commands + [
                "-d",
                job_id,
                "-o",
                f"Out/{sim_name}_{v1_neurons}_test_{i}.out",
                "-e",
                f"Error/{sim_name}_{v1_neurons}_test_{i}.err",
                "-J",
                f"{sim_name}_test_{i}",
            ]
            new_evaluation_script = (
                evaluation_script
                + f"--seed {flags.seed + i} --ckpt_dir {logdir} --restore_from 'Intermediate_checkpoints' --run_session {i}"
            )
            new_evaluation_command = (
                new_evaluation_command + ["--wrap"] + [new_evaluation_script]
            )
            eval_job_id = submit_job(new_evaluation_command)
            eval_job_ids.append(eval_job_id)

    # Final evaluation with the best model
    final_evaluation_command = evaluation_commands + [
        "-d",
        job_id,
        "-o",
        f"Out/{sim_name}_{v1_neurons}_test_final.out",
        "-e",
        f"Error/{sim_name}_{v1_neurons}_test_final.err",
        "-J",
        f"{sim_name}_test_final",
    ]
    final_evaluation_script = (
        evaluation_script
        + f"--seed {flags.seed + i} --ckpt_dir {logdir} --restore_from 'Best_model' --run_session {i}"
    )
    final_evaluation_command = (
        final_evaluation_command + ["--wrap"] + [final_evaluation_script]
    )
